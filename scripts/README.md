...
| `gen_dep_graph.py` | `npm run graph` | Regenerates the committed `docs/catalog/module-graph` bilingual pair plus its consistency record — a mermaid dependency graph built with grimp from the `root_packages` declared in `import-linter.ini` (direct imports aggregated per top-level package, sorted output). With `--check` (exposed as `npm run graph:check`) it exits 1 when any of the three files is missing or stale. |
