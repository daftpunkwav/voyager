...
TOOL_MECHANISMS: dict[str, str] = {
    ...
    "session/actions.py": "one action dispatch (session_action + interaction guard) shared by the session capability and tool",
    "plan/plan_ops.py": "plan-gate operations shared by the plan capability and tool (human exit vs agent submit)",
    "skill/skill_ops.py": "skill load/propose operations shared by the skill capability and tool",
