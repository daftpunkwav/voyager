"""Tests for context engineering: layered system-prompt assembly, compression,
page awareness, and on-demand load auditing.
"""
...
class TestBuilder:
    def test_layer_order(self, tmp_path) -> None:
        ...
        assert order == sorted(
            order
        )  # layer order: rules -> persona -> style -> profile -> task -> page
