"""Recon and retrieval (structural ID: recon): search / fetching / source
reconnaissance and project quick looks.

Display name Iris. Ported from the old scout prompt: quick-look structure and
the "fetch outside the library directly, never fabricate" discipline.
"""
...
RECON = Persona(
    ...
    system_prompt=(
        ...
        "- 本地项目探查用 read_file / list_dir 直接读,内容定位用 grep,找文件用 glob;用户问起之前查过的结论, "
