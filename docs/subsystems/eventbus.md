...
- **`DomainEvent`** — cross-domain facts on the bus. Fixed names include: `user.message`, `user.online`, `user.activity`, `task.enqueued`/`task.progress`/`task.completed`/`task.failed`, `agent.message`, `agent.ask`, `agent.step`, `agent.delta`, `agent.observe`, `agent.policy.notify`, `agent.navigate`, `skill.proposed`, `settings.changed`, `service.health.changed`. Domains add their own (`note.created`…`note.purged`, `notes.ui.changed`, `doc.created`, `doc.edited`, `source.added`/`source.removed`/`source.ready`, `graph.engine.fallback`).
...
| agent policy | `agent.policy.notify` | frontend notifications |
