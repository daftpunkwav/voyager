...
`clients/` mounts external MCP servers (`agent.mcp.servers`, approval-gated): remote tools become `mcp__<server>__<tool>` agent tools (`clients/mount.py`), `dimension="app"`, registered only after approval. `McpSession` (`clients/session.py`) speaks JSON-RPC 2.0 over stdio or HTTP with a 30 s call timeout.
