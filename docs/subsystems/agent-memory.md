...
`distill.py` — `Distiller.maybe_distill()` fires every `agent.memory.distill_interval` turns: one LLM call producing strict JSON that lands in profile/semantic writes. Distillation reads working memory, never the full log.

## Read policy

`read_policy.py` — `render_relevant_recall` is the resident relevance layer of the system prompt: recalled facts render content-addressed (same facts → same bytes) so the prompt prefix stays cache-stable.
...
`skills/loader.py` — `SkillLoader(roots)` scans roots for `<name>/SKILL.md`. `index()` returns name + first-line description (≤120 chars) only; `full_text(name)` loads on demand (unreadable → `KeyError`). `add_root`/`remove_root` support plugin hot-load/unload. Built-in roots: `agent/src/agent/skills/builtin` and `workspace/skills`.
