...
- **C sidecar** — `engines/c/client.py` `CEngineClient` over HTTP, default `http://127.0.0.1:8123` (`graph.engine.c_url`). The sidecar is a separate process (default port 9750; see `engines/c/core/README.md`) and is **not started automatically** — in `auto` mode the adapter falls back to Python unless its health probe answers. Only `/api/index`, `/api/index-status` and `/api/project-health` are exercised from Voyager today.
...
- `pipelines/code/analyze.py` — `analyze_repo`: engine index → export into the canonical store with `source="code"`, then automatic relation analysis. Note: the export step calls `export_graph`, which only the Python engine implements — with the C sidecar selected (and reachable) this step fails; known gap while the sidecar is dormant.
