...
External MCP servers (`agent.mcp.servers`) mount their tools as `mcp__<server>__<safe-name>` (`agent/src/agent/mcp/mount.py`), `dimension="app"`, registered only after user approval.
