...
# The UI asset service has been removed (Voyager frontend lives in apps/web);
# --with-ui no longer has a dedicated build target.
if $WITH_UI; then
    echo "build.sh: warning: --with-ui is deprecated (frontend asset service removed); building the standard target" >&2
fi
