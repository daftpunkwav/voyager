...
from transformers import AutoModel, AutoTokenizer  # after the os.environ setup above
...
def extract_embeddings(
    model, tokenizer, tokens: list, device: str, batch_size: int = 64, checkpoint_path: str = None
) -> np.ndarray:
...
def main():
    ...
    parser.add_argument(
        "--output-dir", default="vendored/nomic", help="Output directory (default: vendored/nomic)"
    )
    ...
    print(
        f"step 3: extracting embeddings ({len(filtered_tokens)} tokens, batch_size={batch_size})..."
    )
    t0 = time.time()
    ...
    if not args.skip_attention:
        print(
            f"step 4: simulated attention (K={SIM_ATTENTION_K}, "
