...
# Step 5e: the `daemon start --open` UI readiness check depends on the
# engine-with-ui target and tests/fixtures/ui-readiness; the frontend asset
# service has been removed (Voyager frontend lives in apps/web), so this step
# and its dependencies are dead; the comment is kept for context.
