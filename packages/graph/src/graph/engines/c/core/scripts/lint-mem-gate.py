...
WHITELIST = Path(
    os.environ.get("LINT_MEM_WHITELIST", Path(__file__).resolve().parent / "lint-mem-whitelist.txt")
)
...
FINDING_RE = re.compile(
    r"^(?P<file>[^:]+):(?P<line>\d+):\d+:\s+(?:error|warning):\s+(?P<msg>.*?)\s*\[(?P<check>[\w.-]+)\]\s*$"
)
...
def function_spans(path):
    ...
    for idx, line in enumerate(clean):
        if depth == 0 and line[:1] not in ("", " ", "\t", "#"):
            ...
            if (
                idx == 0
                or clean[idx - 1].strip() == ""
                or clean[idx - 1].rstrip().endswith(("}", ";"))
            ):
                decl_start = idx
        opened = line.count("{")
        closed = line.count("}")
...
def main():
    ...
    if used:
        sys.stderr.write(
            f"=== memory gate clean ({len(used)} argued false positive(s) suppressed) ===\n"
        )
    else:
