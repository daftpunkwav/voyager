/*
 * main.c — entry point for the Voyager graph C engine (source migrated from
 * graph-engine, MIT).
 *
 * Modes:
 *   (default)           MCP: stdin/stdout JSON-RPC 2.0
 *   cli <tool> <json>   one-shot tool call, prints the result
 *   --version / --help  version and usage
 *   --ui=true/false     enable/disable the HTTP service (layout/index/rpc;
 *                       settings persist)
 *   --port=N            HTTP port (default 9750, persists)
 *   --tool-profile=...  restrict the exposed tool surface
 *
 * Long-lived MCP/hook frontends act as thin clients of the account-level
 * daemon; one-shot CLI runs use an isolated local service.
 */
#ifdef _WIN32
 ...
 * non-ASCII CLI argument (e.g. a repo path like café_über_repo) is mangled before the
...
int main(int argc, char **argv) {
    ...
    /* SEC-003: emit a prominent warning when the indexing boundary is not
     * declared. fail-closed is enforced at the engine_workspace_root_allowed
     * layer — indexing requests are always rejected when ENGINE_ALLOWED_ROOT
     * is unset and no allow-root grant exists. This warning lets an operator
     * who started the process manually know immediately that the boundary is
     * missing, instead of discovering it only after indexing is denied. */
    {
        ...
        if (!allowed || !allowed[0]) {
            (void)fprintf(stderr,
                          "warning: ENGINE_ALLOWED_ROOT is not set — no indexing "
                          "boundary declared; indexing requests will be rejected "
                          "(fail-closed). Set the variable or run "
                          "graph-engine allow-root <path> to declare an indexable root.\n");
        }
    }
